...
# --- склейка по почти тому же тексту и по фотографии (02.09.2026) ---
LONG = ("Продаю холодильник Samsung двухкамерный, состояние отличное, "
        "работает тихо, забрать самовывозом в Джомтьене, цена 8500 бат, "
        "торг уместен, пишите в личку")
NEAR = ("Продаю холодильник Samsung двухкамерный, состояние отличное, "
        "работает тихо, забрать самовывозом в Джомтьене, 8500 бат, торг "
        "уместен, пишите в личку, срочно")
OTHER = ("Сдаю студию в Пратамнаке на длительный срок, есть бассейн и "
         "спортзал, депозит два месяца, свободна с первого числа")
PH1 = "1010" * 16
PH2 = ("1010" * 16)[:-3] + "011"   # три бита разницы — то же фото
PH3 = "0011" * 16                   # совсем другое фото

dedup.remember("masha", LONG, "Барахолка", [PH1])
check("переписанный тем же продавцом пост — та же строка",
      dedup.is_dup("masha", NEAR, "Барахолка"))
check("другое объявление того же продавца — новая строка",
      not dedup.is_dup("masha", OTHER, "Барахолка"))
check("та же фотография, но цена другая — новая строка",
      not dedup.is_dup("masha", LONG.replace("8500", "7900"), "Барахолка", [PH1]))

NOPRICE = "Продаю велосипед горный, рама алюминий, катался мало, самовывоз"
NOPRICE2 = "Велик отдам в хорошие руки, Пратамнак, состояние живое, торг"
dedup.remember("kolya", NOPRICE, "Барахолка", [PH1])
check("то же фото при другом тексте — та же строка",
      dedup.is_dup("kolya", NOPRICE2, "Барахолка", [PH2]))
check("другое фото и другой текст — новая строка",
      not dedup.is_dup("kolya", NOPRICE2, "Барахолка", [PH3]))
check("то же фото у ДРУГОГО продавца — новая строка (это работа сайта)",
      not dedup.is_dup("sveta", NOPRICE2, "Барахолка", [PH1]))

dedup.save()
with open(dedup.DEDUP_FILE, encoding="utf-8") as f:
    saved = json.load(f)
check("файл сохранён в третьем формате", saved.get("v") == 3, str(saved)[:120])
