...
# Услуга без цены = «цена договорная» (решение Савелия 07.09.2026).
# Выключатель на случай, если послабление начнёт тащить на сайт мусор:
# SERVICES_NO_PRICE=0 в .env вернёт прежнее поведение («цена не разобрана»).
# Им же пользуется замер services_hits.py, чтобы показать «до и после».
SERVICES_NO_PRICE = os.environ.get("SERVICES_NO_PRICE", "1") != "0"
...
def build_listing(text: str, schema: dict | None = None) -> dict:
    ...
    if price is None and not is_free and not is_negotiable:
        ...
        if category == "services" and SERVICES_NO_PRICE:
            is_negotiable = True
        else:
