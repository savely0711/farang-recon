...
Это ПЕРВЫЙ слой разбора. Он бесплатный и работает всегда, а поверх него идёт
ИИ (`realty_ai.py`), который уточняет разбор и добавляет то, чего правилами не
взять: агентство или частник и название конторы. Если ИИ не ответил (кончился
баланс Anthropic, сбой сети), в таблицу попадает результат ЭТОГО файла — то
есть контур продолжает собирать объявления при любой погоде.

Отчёт о том, как часто правила справляются сами: `python3 realtytry.py 200`.
