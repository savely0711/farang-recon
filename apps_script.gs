 ...
 *   «На сайте»    — состояние объявления на сайте (ставит авто-подготовка, уточняет
 *                  ночная сверка):
...
var SITE_REVIEW = 'Ждёт модератора';// лежит в очереди на проверку (в т.ч. сразу после прогона)
...
var SITE_VALUES = [SITE_CATALOG, SITE_REVIEW, SITE_OFF, SITE_GONE, SITE_FAIL];
// Значение из первых версий: тогда «Опубликовано» означало всего лишь «ушло на
// сайт», то есть ровно «Ждёт модератора». Встретив его, молча переводим —
// отдельным состоянием оно больше не живёт (решение Савелия 20.08.2026).
var SITE_LEGACY_OK = 'Опубликовано';
...
function _ensureColorRules(sh) {
  ...
  var sitePairs = [
    [SITE_CATALOG, PR_GREEN], [SITE_REVIEW, PR_BLUE], [SITE_OFF, PR_YELLOW],
  ];
...
function _safeSite(v) {
  ...
  if (s === SITE_LEGACY_OK) return SITE_REVIEW;
