...
Объявления без ника тоже записываем — как рыночные данные; писать им некому.
  Колонки: Ник | Ссылка | Канал | Категория | Дата | Описание | Написали? |
           Присутствие | На сайте | Тип продавца
...
Кто какой колонкой управляет (решение Савелия 16.08.2026):
  ...
  «На сайте», «Тип продавца» — см. план мини-CRM.
...
class Sheet:
    ...
    def set_site_result(self, link: str, value: str) -> bool:
        ...
        (ключ — ссылка на пост). Значения: SITE_REVIEW «Ждёт модератора» (синим,
        объявление ушло на сайт и ждёт человека), SITE_FAIL «Не вышло» (красным),
        пустая строка — снять пометку и вернуть объявление в очередь. Дальше
        состояние уточняет ночная сверка (`syncsite.py`): «В каталоге», «Снято»,
        «Удалено».
